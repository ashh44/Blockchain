...
// Import the ChainlinkClient contract from the Chainlink library
import "@chainlink/contracts/src/v0.8/ChainlinkClient.sol";

contract MyDataContract is ChainlinkClient {
    ...
    function getData() external returns (string memory) {
        if (isOnChain) {
            ...
            return getOffChainData();
        }
    }

    // Function to get on-chain data
    function getOnChainData() internal view returns (string memory) {
        return onChainData;
    }

    // Function to get off-chain data using Chainlink
    function getOffChainData() internal returns (string memory) {
        // Create a Chainlink request
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);

        // Set the URL to fetch off-chain data (replace with your specific URL)
        request.add("get", "https://api.example.com/eth-usd-price");
        ...
        // Return a placeholder result for now (actual result will be emitted in the fulfill function)
